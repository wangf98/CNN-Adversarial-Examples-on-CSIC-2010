...
class Net(nn.Module):
    ...
    def classify(self, x):
        ...
        outputs = softmax(outputs)  # probability of predicted classes
...
with open("../data/not_attack.pkl", "rb") as f:
    samples = pickle.load(f)

logs = samples["log"]
label_trues = samples["labels"]
logs_adversarial = []
...
#ff = open("../datas/perfect_adversarial_logs.txt","w")

with open("../datas/perfect_adversarial_logs.txt","w") as ff:
#while num_adversarial_logs<=10000:
    totalMisclassifications = 0
    for log, label_true in tqdm(zip(logs, label_trues)):  # enumeration of the dataset
        if label_true == 1:
            ...
            probability, label_pred_adversarial = t.max(net(log_adversarial).data, 1)
            print(probability)

            if label_true != label_pred:
                totalMisclassifications = totalMisclassifications+1  # filter the samples which are wrongly predicted
                ...
                #  Visualisation
                if label_true != label_pred_adversarial and probability >= 0.8:
                    num_adversarial_logs = num_adversarial_logs+1
                    if num_adversarial_logs>=10000: break
                    print(num_adversarial_logs)
                    for i in range(200):
